...
const CurrencyNav = () => {
    ...
    const fetchData = () => {
        var options = {
            ...
            params: {vs_currency: `${currency}`, page: '1', per_page: '100', order: `${order}`},
            headers: {
    ...
    const updateInput = (event) => {
        ...
        const filtered =  coins.filter(coin => {
            return coin.name.toString().toLowerCase().includes(input.toString().toLowerCase())
        })
        setCoins(filtered)
    }

    const clickHandler = curr => {
        setCurrency(curr)
    }
    ...
    return (
        <>
            <div className='currency-nav'>
                <Currency onChange={clickHandler}/>
                <div className="header-search-bar">
                    <span className="header-search-input-container">
                        <div className="form-container">
                            ...
                            <input type="text" inputMode='search' className='input-search' placeholder="Search" input={input} onChange={updateInput}/>
                        </div>
                    </span>
                </div>
                <div className="list-sort">
                    <div onClick={() => setOrder('market_cap_asec')} className="list-sort-item-pair">
                        <span className="not-selectable">Pair </span>
                        <i class="fas fa-arrow-up arrow"></i>
                    </div>
                    <div onClick={() => setOrder('total_volume_asec')} className="list-sort-item-vol">
                        <span className="not-selectable">Vol </span>
                        <i class="fas fa-arrow-up arrow"></i>
                    </div>
                    <div onClick={() => setOrder('market_cap_change_percentage_24h_desc')} className="list-sort-item-change">
